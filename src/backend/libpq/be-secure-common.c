...
bool check_ssl_key_file_permissions(const char *ssl_key_file,
  ...
  if (!S_ISREG(buf.st_mode)) {
    ereport(loglevel, (errcode(ERRCODE_CONFIG_FILE_ERROR),
  ...
  if (buf.st_uid != geteuid() && buf.st_uid != 0) {
    ereport(loglevel, (errcode(ERRCODE_CONFIG_FILE_ERROR),
                       errmsg("private key file \"%s\" must be owned by the "
    ...
    ereport(loglevel,
            (errcode(ERRCODE_CONFIG_FILE_ERROR),
             errmsg("private key file \"%s\" has group or world access",
                    ssl_key_file),
             errdetail("File must have permissions u=rw (0600) or less if "
                       "owned by the database user, or permissions u=rw,g=r "
